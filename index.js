...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASSWORD}@cluster0.tqy0s.mongodb.net/?retryWrites=true&w=majority`;
console.log(uri);
...
async function run() {
   try {
      ...
      const productCollection = client.db("tech-part").collection("products");
      const orderCollection = client.db("tech-part").collection("orders");
      const userCollection = client.db("tech-part").collection("users");
      const reviewCollection = client.db("tech-part").collection("reviews");
